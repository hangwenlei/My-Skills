...
| 休眠的看板条目 | frontmatter 记录的**进入休眠时间**距今超过 30 天 | 删除该条目文件 |
| 分支现场 | 分支仍在、未合并，但其 `git log -1` 超过 14 天（由 L1 的 tier 3 转入） | 在其 frontmatter 标记休眠并写入进入休眠时间 |
| 休眠的分支现场 | frontmatter 记录的**进入休眠时间**距今超过 30 天 | 决策提炼后删除，见上方「分支现场删除前必须先提炼决策」 |
...
1. **迁移**：若「老项目迁移」的识别表命中任一行，按该节规则完成迁移（含
   迁移锚点判断：主干才执行完整拆分）。
2. **执行淘汰决议**：先执行删除类动作——删除前必须先完成决策提炼并入稳定层
   （见「分支现场删除前必须先提炼决策」）；再执行休眠标记类动作——在对应
   条目或文件自身的 frontmatter 写入休眠标记与进入休眠时间。
...
不存在独立的休眠区文件；该字段供 L2 的 30 天删除判据使用，不能用最后更新时间代替。
